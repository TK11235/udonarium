...
2d+{ライダー}+({知力}/6) 【ライダー観察判定*】足跡追跡/異常感知/危険感知/探索/罠回避 (*要探索指令)
...
2d+{${skillName}}+({知力}/6)${mod}+{魔法行使} 【${magic.alias}行使判定】\n`;
      cpBattle += magic.attack
        .map(
          iryoku =>
            `k${iryoku}+{${skillName}}+({知力}/6)${mod}+{魔法威力} 【${
              magic.alias
            }威力${iryoku}】\n`
        )
...
`;
      ...
      cpBattle += magic.heel
        .map(
          iryoku =>
            `k${iryoku}+{${skillName}}+({知力}/6)${mod}+{魔法威力}@13 【${
              magic.alias
            }回復${iryoku}】\n`
        )
        .reduce((txt, elm) => txt + elm, '');
