...
(function(Opal) {
  ...
  Opal.add_stubs(['$require', '$setPrefixes', '$new', '$sub', '$changeText', '$checkRoll']);
  
  self.$require("diceBot/NightWizard");
  return (function($base, $super, $parent_nesting) {
    function $SevenFortressMobius(){};
    var self = $SevenFortressMobius = $klass($base, $super, 'SevenFortressMobius', $SevenFortressMobius);

    var def = self.$$proto, $nesting = [self].concat($parent_nesting), TMP_SevenFortressMobius_initialize_1, TMP_SevenFortressMobius_gameName_2, TMP_SevenFortressMobius_gameType_3, TMP_SevenFortressMobius_getHelpMessage_4, TMP_SevenFortressMobius_changeText_6, TMP_SevenFortressMobius_dice_command_xRn_7;

    def.nightWizardDiceBot = nil;
    
    self.$setPrefixes(["\\d*SFM"]);
    
    Opal.defn(self, '$initialize', TMP_SevenFortressMobius_initialize_1 = function $$initialize() {
      var self = this, $iter = TMP_SevenFortressMobius_initialize_1.$$p, $yield = $iter || nil, $zuper = nil, $zuper_i = nil, $zuper_ii = nil;

      if ($iter) TMP_SevenFortressMobius_initialize_1.$$p = null;
      // Prepare super implicit arguments
      for($zuper_i = 0, $zuper_ii = arguments.length, $zuper = new Array($zuper_ii); $zuper_i < $zuper_ii; $zuper_i++) {
        $zuper[$zuper_i] = arguments[$zuper_i];
      }
      
      $send(self, Opal.find_super_dispatcher(self, 'initialize', TMP_SevenFortressMobius_initialize_1, false), $zuper, $iter);
      return (self.nightWizardDiceBot = Opal.const_get_relative($nesting, 'NightWizard').$new());
    }, TMP_SevenFortressMobius_initialize_1.$$arity = 0);
    
    Opal.defn(self, '$gameName', TMP_SevenFortressMobius_gameName_2 = function $$gameName() {
      var self = this;

      return "セブン＝フォートレス メビウス"
    }, TMP_SevenFortressMobius_gameName_2.$$arity = 0);
    
    Opal.defn(self, '$gameType', TMP_SevenFortressMobius_gameType_3 = function $$gameType() {
      var self = this;

      return "SevenFortressMobius"
    }, TMP_SevenFortressMobius_gameType_3.$$arity = 0);
    
    Opal.defn(self, '$getHelpMessage', TMP_SevenFortressMobius_getHelpMessage_4 = function $$getHelpMessage() {
      var self = this;

      return "" + "・判定用コマンド　(nSFM+m@x#y)\n" + "　\"(基本値)SFM(常時および常時に準じる特技等及び状態異常（省略可）)@(クリティカル値)#(ファンブル値)（常時以外の特技等及び味方の支援効果等の影響（省略可））\"でロールします。\n" + "　Rコマンド(2R6m[n,m]c[x]f[y]>=t tは目標値)に読替されます。\n" + "　クリティカル値、ファンブル値が無い場合は1や13などのあり得ない数値を入れてください。\n" + "　例）12SFM-5@7#2　　1SFM　　50SFM+5@7,10#2,5　50SFM-5+10@7,10#2,5+15+25\n"
    }, TMP_SevenFortressMobius_getHelpMessage_4.$$arity = 0);
    
    Opal.defn(self, '$changeText', TMP_SevenFortressMobius_changeText_6 = function $$changeText(string) {
      var TMP_5, self = this;

      
      string = $send(string, 'sub', [/(\d*)SFM/i], (TMP_5 = function(){var self = TMP_5.$$s || this, $a;

      return "" + ((($a = $gvars['~']) === nil ? nil : $a['$[]'](1))) + "NW"}, TMP_5.$$s = self, TMP_5.$$arity = 0, TMP_5));
      return (string = self.nightWizardDiceBot.$changeText(string));
    }, TMP_SevenFortressMobius_changeText_6.$$arity = 1);
    return (Opal.defn(self, '$dice_command_xRn', TMP_SevenFortressMobius_dice_command_xRn_7 = function $$dice_command_xRn(string, nick_e) {
      var self = this;

      return self.nightWizardDiceBot.$checkRoll(string, nick_e)
    }, TMP_SevenFortressMobius_dice_command_xRn_7.$$arity = 2), nil) && 'dice_command_xRn';
  })($nesting[0], Opal.const_get_relative($nesting, 'DiceBot'), $nesting);
})(Opal);
