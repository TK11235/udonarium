...
(function(Opal) {
  ...
  return (function($base, $super, $parent_nesting) {
    ...
    Opal.defn(self, '$magicalogia_parallel_world_scene_table', TMP_MagicaLogia_magicalogia_parallel_world_scene_table_84 = function $$magicalogia_parallel_world_scene_table() {try {
      ...
      table = ["カフェの桌面。FMラジオは、去年解散したはずのバンドの、先日発表した新曲を流している。このシーンに登場しているPCのうち一人は、「時の流れ表」を1回振り、効果を適用する。", $send(self, 'lambda', [], (TMP_81 = function(){var self = TMP_81.$$s || this;
    ...
    Opal.defn(self, '$magicalogia_starlight_scene_table', TMP_MagicaLogia_magicalogia_starlight_scene_table_90 = function $$magicalogia_starlight_scene_table() {try {
      ...
      table = ["あなたの持つ「切符」は無効であると車掌に告げられる。他の乗客の視線を感じる。このシーンに登場しているPCのうち一人は、もう一度「切符」を調達しなければならない。魂の特技の判定を、成功するまで繰り返すこと。", "ごとごと。ごとごと。ガラスより透き通る銀河の水辺を走る。色とりどりの花々が星の光に冴え渡っては通り過ぎて行く。", "ふと目を覚ます。汽車に揺られていつの間にか眠っていたらしい。夢の中で誰かと会っていた気がしてならない。シーン玩家が≪深淵≫の判定に成功すると、ランダムに選んだアンカーとの運命が1点上昇する。", "銀河の水辺にぽつりと浮かぶ島に、目の覚めるような白い立派な十字架がそびえたっている。このシーンに登場しているPCのうち一人は、≪祈り≫で判定を行うことができる。成功すると、以降このゲーム中に発生する魔法災厄が一回だけ無効化される。誰かが判定すると、成否にかかわらず、この効果はなくなる。", "窓から銀河を見つめる、黒い外套を着た少女がいる。あなたに気づくとお菓子を差し出し、にこりと笑って去っていく。シーン玩家はランダムに選んだ魔素を1点獲得する。", "ノイズ混じりの車内放送が「停車駅」への到着を告げる。しばらくすると「汽車」が止まった。角色は「停車駅」で降りることができる。降りる場合、「停車駅」の世界法則を決定し、その法則に従ってシーンを進めること。", "車窓の向こうに、星明りに照らされたススキ野原が青白く広がる。そんな中で、不思議な紋様の書かれた三角標が何本か、ぽつり、ぽつりと立ち尽くしているのが見えた。", "気づくと誰もいない。青いビロウドを張った座席だけが、空間をむなしく彩っている。シーン玩家以外のPCが、このシーンに登場しようとする場合、≪静寂≫で判定を行う。失敗したPCは、このシーンに登場できない。", $send(self, 'lambda', [], (TMP_89 = function(){var self = TMP_89.$$s || this;
    ...
    Opal.defn(self, '$magicalogia_world_low_add_table', TMP_MagicaLogia_magicalogia_world_low_add_table_91 = function $$magicalogia_world_low_add_table() {
      ...
      table = ["世界法則「楽園」。この「停車駅」は美しく、慰めに満ちている。この「停車駅」で降りた〈愚者〉は魅了され、ここで一生を過ごすことになる。「刻印百景」の特技で判定に成功すると、これを押しとどめることができる。この判定は、対象の〈愚者〉一人につき一回しかできない。", "世界法則「夜祭」。星の明かりや提灯を頼りに、終わらないお祭りが続いている。このシーンに登場している角色は、「刻印百景」の特技で判定に成功すると、【手当】の俗呪を使用したとき、消費した魔素1点につき、【魔力】を2点回復できるようになる。", "世界法則「遺棄」。この「停車駅」の周辺は、様々なモノが捨てられている。シーン玩家が「刻印百景」の特技で判定に成功すると、魔法【回想】と同じ効果が一回だけ発生する。誰を対象とするかは、シーンに登場している角色の中から、ランダムに選ばれる。", "世界法則「楽園」。この「停車駅」の周辺は、魔素を帯びた不思議な色合いの羽をもつ蝶が生息している。このシーンに登場している角色は、「刻印百景」の特技で判定に成功すると、任意の魔素を2点獲得できる。失敗すると、ランダムに選んだ変調1つを受ける。", "この「停車駅」の周辺は死んだ〈愚者〉たちの終着点でもある。もしここで降りる〈愚者〉がいれば、その人物は死んでいたことになる。このシーンに登場している角色がその〈愚者〉をアンカーとしている場合、「刻印百景」の特技で判定に成功すると、「疵」になることなく運命欄から消去できる。", "世界法則「魔法門」。他の異境への入り口が存在する。「刻印百景」の特技で判定に成功すると、その魔法門を発見できる。"];
      return self.$get_table_by_1d6(table);
