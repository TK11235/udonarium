...
#TKfix =begin 削除
  ...
  def setCount
    ...
    #TKfix << 
    output = ""
    output = output + "#{@characterName.downcase}" if(@nick != @characterName)
    output = output + "(#{@tagName}) #{@currentValue}";
  ...
  def changeCount
    ...
    output = ""
    output = output + "#{@characterName.downcase}" if(@nick != @characterName)
    output = output + "(#{@tagName}) #{preText} -> #{nowText}";
...
####################          カウンタ一覧         ########################
  ...
  def getPointListAtSameChannel(tagName)
    ...
    #TKfix << 
    output = output +  "#{tagName}:" unless( tagName.empty? )
    ...
    characterInfoList.keys.sort.each do |characterName|
      ...
      characterInfo.keys.sort.each do |currentTag|
        ...
        #TKfix << 
        tagText = tagText + "#{currentValue}"
        tagText = tagText +  "/#{maxValue}" unless( maxValue.nil? )
      end
      
      unless( tagText.empty? )
        #TKfix << 
        output = output + " " unless( output.empty? )
        output = output +  "#{characterName}(#{tagText})"
      end
    end
