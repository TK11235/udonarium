...
MESSAGETEXT
  ...
  def get_damageEffect_table(hitPart, damageStage)
    ...
    when 'E'
      ...
      table = [
        ...
        [2, 'ダメージ修正+20。ノイズ。1シーン、角色ならすべてのロールが、アイテムならそれを使用したロールが1/2となる。'],
        [3, 'ダメージ修正+30。恐怖。1シーン、角色ならすべてのロールが、アイテムならそれを使用したロールが1/4となる。'],
        [4, 'ダメージ修正+50。クラッシュ。角色なら［戦闘不能］。アイテムなら1シナリオ中、使用不可。']
      ]
    when 'B'
      name = 'メカニック損傷効果表：本体'
      table = [
  ...
  def get_mechanicAccident_table(locationType, roc, correction)
    ...
    when 'S'
      name = '水上／水中メカニック事故表'
      table = [
        [3, '横揺れ。次のクリンナッププロセスまで、このメカニックに乗っている角色の行うすべての［メカニック］ロールに1/2の修正が与えられる。'],
  ...
  def get_strategyEvent_chart
    ...
    table = [
      ...
      [57, '狙撃！　司令官角色は胴体に〈物〉155点の固定ダメージを受ける。機動回避は可能。'],
      ...
      [93, '致命的な狙撃！　司令官角色は胴体に〈物〉205点の固定ダメージを受ける。機動回避は可能。'],
