...
MESSAGETEXT
  ...
  def getHitResult(count, damageFunc, tail)
    ...
    #TKfix <<
    totalResultText = totalResultText + "\n ＞ #{hitCount}回命中"
    totalResultText = totalResultText + " 命中箇所：" + getTotalDamage(damages) if( hitCount > 0 )
  ...
  def getDamages(damageFunc, partTable, damages)
    ...
    if( isLrm )
      ...
      #TKfix <<
      resultText = resultText + "[#{dice}] #{damage}点"
    end
    ...
    damagePartCount.times do |damageIndex|
      ...
      #TKfix <<
      resultText = resultText + " " if( isLrm )
  ...
  def getTotalDamage(damages)
    ...
    parts.each do |part|
      ...
      #TKfix <<
      text = ""
      text = text + "#{part}(#{damageCount}回) #{damage}点"
      text = text + " #{criticals.join(' ')}" unless( criticals.empty? )
  ...
  def getHitResultOne(damageText, partTable)
    ...
    #TKfix <<
    result = ""
    result = result + "[#{value}] #{part.gsub(/＠/, '（致命的命中）')} #{damageText}点"
    debug('result', result)
    ...
    if( isCritical )
      criticalDice, criticalText = getCriticalResult()
      #TKfix <<
      result = result + " ＞ [#{criticalDice}] #{criticalText}"
    end
