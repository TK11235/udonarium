...
MESSAGETEXT
  ...
  def checkRoll(base, target, diff)
    ...
    result = result + getRollResultString(diceList, target, diff)
  ...
  def getRollResultString(diceList, target, diff)
    ...
    #TKfix <<
    result = result + "判定値[#{target}] 達成値[#{successCount}]"
    result = result + "+クリティカル[#{criticalCount}]=[#{successTotal}]" if criticalCount > 0
    
    successText = getSuccessResultText(successTotal, diff)
    #TKfix <<
    result = result + " ＞ #{successText}"
