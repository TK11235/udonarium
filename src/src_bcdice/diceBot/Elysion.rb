...
#  温泉 H:Hotsprings／修学旅行 T:School trip／お祭り室 E:festival／潜入調査 U:Infiltration study／
  ...
  def check(base, modify)
    ...
    result = ""
    #TKfix <<
    result = result + "(2D6#{getValueString(base)}#{getValueString(modify)})"
    result = result + " ＞ #{diceTotal}[#{dice1},#{dice2}]#{getValueString(addTotal)} ＞ #{total}"
    
    if dice1 == dice2
      #TKfix <<
      result = result + getSpecialResult(dice1, total)
      return result
    end
    #TKfix <<
    result = result + getCheckResult(total)
  ...
  def getSuccessResult(success)
    ...
    #TKfix <<
    result = " ＞ 成功度#{success}"
    result = result + " ＞ 大成功 《アウル》2点獲得" if success >= @@successMax
  ...
  def getFambleResultText(number, total)
    ...
    #TKfix <<
    result = result + " ／ (#{number - 1}回目のアシストなら)大失敗"
  ...
  def getDateBothResult(type, pc1, pc2)
    ...
    #TKfix <<
    result = result + getDateResult(type, number, pc1, pc2)
