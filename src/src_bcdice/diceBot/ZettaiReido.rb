...
INFO_MESSAGE_TEXT
  ...
  def roll2DR(baseAvility, modText, diffValue)
    ...
    #TKfix <<
    output = ""
    output = output +  "(#{baseAvility}-2DR#{modText}#{diffText})"
    output = output + " ＞ #{baseAvility}-#{diceTotal}[#{diceText}]#{modText}"
    
    total = baseAvility - diceTotal + mod
    output = output + " ＞ #{total}"
    
    successText = getSuccessText(diceTotal, total, diff)
    output = output + successText
    
    darkPointText = getDarkPointResult(total, diff, darkPoint)
    output = output + darkPointText
