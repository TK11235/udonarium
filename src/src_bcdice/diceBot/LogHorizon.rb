...
MESSAGETEXT
  ...
  def getTresureDiceCommandResult(command)
    ...
    type = reg1
    rank = reg2.to_i
    is_choice = (reg2.empty? || (not reg4.nil?))
    modifyText = reg3
    modify = getValue(modifyText, 0)
    is_prize = (reg4 == "$")
    dice_value = nil
  ...
  def getEastalDiceCommandResult(command)
    ...
    return command if (reg1.empty? && reg2.empty? && reg3.nil?)

    rank = reg1.to_i
    is_choice = (reg1.empty? || (not reg3.nil?))
    modifyText = reg2
    modify = getValue(modifyText, 0)
    is_fix_roll = (not reg3.nil?)
    dice_value = reg4
