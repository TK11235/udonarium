  ...
  def get2d6TableInfo(command)
    ...
    when /CrimeIET/i
      ...
      table = %w{
        ...
        謎の筆記発見……このターゲットについて調べている間、このトピックのタグをチーム全員が所有しているものとして扱う
    ...
    when /LoveIET/i
      ...
      table = %w{
        ...
        有力者からの突然のご指名。チームの仲間を１人、ランダムに決定する。差し出すなら、その角色は次の１ターン行動できない代わり、その後にその角色の〔恋愛〕と同じ数の上位リンクを得る
        愛する人の死。自分に恋人がいれば、１人選んで、その角色を死亡させる。その代わり情報ゲット！
      }
    when /CultureIET/i
      name = "情報イベント表／〔教養〕:"
      table = %w{
        ...
        図書館で幻の書物を発見。上位リンクを８つ得る。角色卡のメモ欄に<クトゥルフ神話知識>、SANと記入し、それぞれ後ろに＋５、－５の数値を書き加える
    ...
    when /CrimeIHT/i
      ...
      table = %w{
        ...
        頼れる協力者のもとへ行くと、彼（彼女）の無惨な姿が……自分の持っている現在のセッションに参加していない角色１体を選び、〔肉体点〕を０にする。そして、致命傷表(p.61）を振ること
      }
    when /LifeIHT/i
      name = "情報ハプニング表／〔生活〕:"
      table = %w{
    ...
    when /G(eneral)?A(ccident)?T/i
      ...
      table = %w{
        ...
        最悪の大事故。ケチャップどころではない！　〔犯罪〕で難易度９の判定を行う。失敗したら、ムーブ判定を行った角色は3D6点のダメージを受け、ケチャップから脱落する。判定に成功すればギリギリ難を逃れる。特に何もなし。
        もうダメだ……。絶望感が襲いかかってくる。後３ラウンド以内にケリをつけないと、あなたが追う側なら自動的に逃げる側が勝利する（逃げる側なら追う側が勝利する）
        まずい、突発事故だ！　ムーブ判定を行った角色は、1D6点のダメージを受ける。
        一瞬ひやりと緊張が走る。　ムーブ判定を行った角色は、〔精神点〕を２点減少する。
        スランプ！　思わず足踏みしてしまう。ムーブ判定を行った者は、ムーブ判定に使用した能力値を使って難易度７の判定を行うこと。失敗したら、ムーブ判定を行った角色は、ケチャップから脱落。成功しても、あなたが追う側なら逃げる側のコマを１マス進める（逃げる側なら自分を１マス戻す）
        ...
        苦しい状態に追い込まれた。ムーブ判定を行った角色は、今後のムーブ判定で成功度が－１される。
        頭の中が真っ白になる。〔精神点〕を1D6減少する。
      }
    when /R(omance)?F(umble)?T/i
      name = "ロマンスファンブル表:"
      table = %w{
        みんなあいそをつかす。自分のトリコ欄の角色の名前をすべて消すこと
        ...
        修羅場！　対象とは別にトリコを所有していれば、その角色が現れ、あなたと対象に血戦をしかけてくる
        恋に疲れる。自分の〔精神点〕が1D6点減少する
        甘い罠。あなたが対象のトリコになってしまう
        平手うち！　自分の〔肉体点〕が1D6点減少する
        浮気がばれる。恋人関係にあるトリコがいれば、その角色の名前をあなたのトリコ欄から消す
    ...
    when /FumbleT/i
      name = "命中判定ファンブル表:"
      table = %w{
        自分の持ち物がすっぽぬけ、偶然敵を直撃！　持っているアイテムを１つ消し、ジオラマ上にいる角色１人をランダムに選ぶ。その角色の〔肉体点〕を1D6ラウンドの間０点にし、行動不能にさせる（致命傷表は使用しない）。1D6ラウンドが経過し、行動不能から回復すると、その角色の〔肉体点〕は、行動不能になる直前の値にまで回復する
        敵の増援！　「三下(p.125）」が1D6体現れて、自分たちに襲いかかってくる（DDは、この処理が面倒だと思ったら、ファンブルした角色の〔肉体点〕を1D6点減少させてもよい）
        お前のいるマスに「障害物」が出現！　そのマスに障害物オブジェクトを置き、そのマスにいた角色は全員２ダメージを受ける（セーブ不可）
        ...
        誤って別の目標を攻撃。目標以外であなたに一番近い角色に４ダメージ（セーブ不可）！
        誤って自分を攻撃。３ダメージ（セーブ不可）！
        今使っている武器が壊れる。アイテム欄から使用中の武器を消すこと。銃器を使っていた場合、暴発して自分に６ダメージ！　武器なしの場合、体を傷つけ３ダメージ（共にセーブ不可）！
        「制服警官(p.129）」が１人現れる。その場にいる角色をランダムに攻撃する
        最悪の事態。〔肉体点〕を０にして、その角色は行動不能に（致命傷表は使用しない）
      }
    when /FatalT/i
      name = "致命傷表:"
      table = %w{
    ...
    when /AfterT/i
      name = "その後表:"
      table = %w{
        ここらが潮時かもしれない。2D6を振り、その目が自分の修得している代償未満であれば、その角色は引退し、二度と使用できない
        ...
        思わず意気投合。今回登場したNPC１人を選び、その角色との【コネ】（p.95）を修得する
        ...
        〔性業値〕が１以下、もしくは１３以上だった場合、その角色は大阪の闇に消える。その角色は引退し、二度と使用できない
      }
    when /KusaiMT/i
      name = "臭い飯表:"
      table = %w{
        ...
        精神異常を訴え、無罪に。しかし、アーカム・アサイレムに移送され、1D6回別の角色でセッションを行うまで、その角色を使用できない。キジルシの代償の中から、ランダムに１つの代償を得る。
        起訴されて有罪に。海上刑務所行き。1D6回別の角色でセッションを行うまで、その角色を使用できない。【前科】の代償を得る。
        起訴されて有罪に。海上刑務所行き。2D6回別の角色でセッションを行うまで、その角色を使用できない。【前科】の代償を得る。
        起訴されて有罪に。海上刑務所行き。終身刑。その角色は引退する。
      }
    when /EnterT/i
      name = "登場表:"
      table = %w{
        「こっから先にはいかせないぜ」　【仇敵】がいれば現われ、血戦が始まる。現在の血戦、もしくはケチャップが終了したら、処理を行うこと。
        「待たせたな、みんな！」　ジオラマの好きな場所に自分の角色を配置する。
        おっと、鉢合わせ。ランダムにジオラマ上の敵を１体選ぶ。選んだ敵と同じマスに、その角色を配置する。
        全力ダッシュで駆けつける！　〔肉体点〕を1D6点消費すれば、ジオラマの好きな場所に自分の角色を配置する。そうでなければ、登場できない。
        裏道を歩いていたら、偶然その場所にでくわした。DDはジオラマの好きな場所にその角色を配置する。
        「キキィー！」　もしもその角色が乗物を装備していれば、DDはジオラマの好きな場所にその角色を配置する。そうでなければ、登場できない。
        ……間に合ったみたいだな。仲間を１人選び、その角色と同じマスに自分の角色を配置する。
        ラッキー、「ジャリ銭」を拾った。……と、そんな場合じゃないよな。
        をっと、お前の好物だ。〔性業値〕判定を行え。「律」ならもう一回、登場表を振ることができる。それ以外なら、角色を配置できない。
    ...
    when /BudTT/i
      ...
      table = %w{
        ...
        目の前にいる人物が非常にいとおしく思えてくる。同じ場所にいる角色の中からランダムに１人選ぶ。1D6ターンの間、その角色のトリコになる。
    ...
    when /GetkT/i
      ...
      table = %w{
        ...
        《フォークリフト》（基本76p、乗物）
        《RPG-7》（基本74p、武器）
        倒された角色は、致命傷表を振り、まだ生きていれば、その角色を倒した者のトリコになる。
        「先にイッてるぜ」その角色１体を倒した者に経験点が１点与えられる。
      }
    when /PayT/i
      name = "落とし前表:"
      table = %w{
