...
INFO_MESSAGE_TEXT
  ...
  def sinobigami_fumble_table(string)
    ...
    when /KFT/
      ...
      table = [
        ...
        'しまった！　好きな忍具を１つ失ってしまう。',
        '情報が漏れる！　あなた以外の角色は、あなたの持っている【秘密】か【居所】の中から、好きなものをそれぞれ一つ知ることができる。',
      ...
      table = [
        '何か調子がおかしい。そのサイクルの間、すべての行為判定にマイナス１の修正がつく。',
        'しまった！　好きな忍具を１つ失ってしまう。',
        '情報が漏れる！　このゲームであなたが獲得した【秘密】は、他の角色全員の知るところとなる。',
        '油断した！　術の制御に失敗し、好きな【生命力】を１点失う。',
  ...
  def sinobigami_metamorphose_table()
    ...
    table = [
      ...
      '妖魔化している間、この角色の攻撃に対する回避判定と、この角色の奥義に対する奥義破り判定にマイナス1の修正がつく。'
    ]
    total_n, = roll(1, 6)
 ...
 2:どこから紛れ込んできたのか。シーン玩家が1D6を振って3以下ならナタを持った少女、4以上なら冬篭りに備えた熊が襲ってくる。シーンに登場した角色は、少女なら《刀術》・熊なら《鳥獣術》で判定し、失敗すると接近戦ダメージを1点受ける。
...
TABLE_TEXT_END
      ...
      'CLST' => {
        ...
        :type => '1D6',
        :table => <<'TABLE_TEXT_END'
瘴気に晒され続けたことであなたの身体が妖魔へと変貌する。《封術》の判定に失敗したシーン玩家は、背景：劣勢因子と背景：魔人を獲得する。
...
TABLE_TEXT_END
      ...
      'HC' => {
        ...
        :type => '2d6',
        :table => <<'TABLE_TEXT_END'
深い闇が辺りを覆う。何度目かの夜……いつまでこの試験は続くのだろう。このシーンに登場する角色は《生存術》で判定を行い、失敗すると集団戦ダメージを1点受ける。
...
TABLE_TEXT_END
      ...
      'HS' => {
        ...
        :type => '1d6',
        :table => <<'TABLE_TEXT_END'
あなたは心地よいベッドの中で、恋人の肌のぬくもりを感じながら微睡んでいる。恋人は「そろそろ起きる時間」とベッドからすり抜ける。目を開けると、そこには裸のナビ角色がいた。そして、ナビ角色はあなたに優しく口づけをした。柔らかな感触が、あなたの記憶を掘り起こす。
...
TABLE_TEXT_END
      ...
      'NTST' => {
        ...
        :type => '2D6',
        :table => <<'TABLE_TEXT_END'
どこから紛れ込んできたのか。ナタを持った少女がこちらに迫ってくる。あっそぼうよぉ。シーンに登場した角色は《刀術》で判定し、失敗すると接近戦ダメージを1点受ける。
 3:暗い夜の森の中、月明かりのみが周囲を照らす。忍が動くにはいい時間だ。
