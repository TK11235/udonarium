...
class BCDice
  ...
  def setMessage(message)
    ...
    pattern = "\\A\\s*(?:#{$OPEN_DICE}|#{$OPEN_PLOT})\\s*\\z"
    openPattern =Regexp.new(pattern, Regexp::IGNORECASE)
  ...
  def addPlot(arg)
    ...
    pattern = "#{$ADD_PLOT}[:：](.+)"
    unless(Regexp.new(pattern, Regexp::IGNORECASE) =~ arg)
      debug("addPlot exit")
      return
    end
    plot = $1
  ...
  def recievePublicMessageCatched(nick_e)
    ...
    pattern = "(^|\\s+)#{$OPEN_PLOT}(\\s+|$)"
    if(Regexp.new(pattern, Regexp::IGNORECASE) =~ @message)
